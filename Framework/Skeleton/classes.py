...
def dir_to_loc(direction):
    ...
    print('new location is either a warning or a random:', new_loc) if type(new_loc) is dict else print('returning new loc, value is:', new_loc) if debug else None
    return new_loc
...
def condense_and_split(p_input):
    ...
    while i < len(final):
        ...
        if final[i] in ands:
            ...
            if next_word_is_verb or next_word_is_dir or final[i+1] in ('save', 'exit', 'resume', 'load'):
                index_of_sublist += 1
                separated_cmds.insert(index_of_sublist, [])
        if regular_appendage and final[i] != 'times':  # remove the word times in a case of go north 5 times
...
def pos_tagger(command):
            ...
            for dictionary in COMMANDS[dict_type]:
                ...
                for cmd in multi_word_cmds:
                    ...
                    if partials_in_usr_cmd == cmd:
                        ...
                        double_partials = ('PFVB-ALL/ITEM-PFVB', 'PVB-ITEM-PVB', 'PFVB-OBJ/ALL-PFVB')
                        single_partials = ('PFVB-ALL/ITEM', 'PVB-ITEM', 'PFVB-OBJ/ALL')
                        if any(structure in label for structure in double_partials):
                            ...
                            second_word_is_item_or_all = check_for_item(second_word) or second_word in ('all', 'everything')
    ...
    def tag_word(index):
        ...
        def check_for_special_cmd(word):
            special_commands = ['save', 'load', 'quit', 'inventory', 'diagnose', 'look']
        ...
        special_words = [[('all', 'everything'), 'ALL'],
                         [('with', 'using'), 'WITH'],
    ...
    if any(partial_tag in str(tagged) for partial_tag in ('PFVB', 'PVB', 'POBJ', 'PLVB')):
        ...
        for order in ('normal', 'reverse'):
            ...
            if typ == 'VERB':
                if next_word in ('all', 'everything'):
                    if user_command[word_index+2] == next_multiword:  # if the word after the object
        ...
        if reaction_subdict is not None:  # if they passed in a value for reaction_subdict...
            ...
            if type(reaction_info) is dict:  # if reaction_info is a str, so just a plain string reaction,
                ...
                if not reaction_info.get(reaction_subdict):  # ...but there is no reaction_subdict key
                    raise YamlFormatError(f"There is no {reaction_subdict} key in the passed-in reaction info, which is: {reaction_info}")
                else:
...
class Location:
    ...
    def narrate(self, typ=None):
        ...
        if self.visited == 0 or typ == 'long':
            ...
            if self.objects:
                descriptions = [OBJECTS[obj].short_description for obj in self.objects]
...
class Player:
    ...
    def describe(self, *things):
        ...
        for what in things:
            if what == 'health':
                ...
                LOCATIONS[player.active_location].narrate('long')
    ...
    def take(self, *references, label):
        ...
        for reference in objects:
            ...
            # is here just in case there is no custom message.
                ...
                print_big('in locked container container is:' + str(container))
                print_big('items_in_locked_container are:' + str(items_in_locked_container))
                if container:
            ...
            # check if player asked to take something from a container and the thing is not in the container
                ...
                print(f"{reference} is in open container: {in_open_container}")
                if obj_is_here:
    ...
    def throw(self, *references, label):
    ...
    def place(self, *references, label):
    ...
    def goto(self, *locations, long_or_short=None):
        ...
        for i in range(len(locations)):
            going_to = True
            for loc in locations[i]:  # loop twice in case a (N,E) - northeast - was passed in. If locations[i] is just one
                # letter, as in 'S', then it will loop over the letter character -- but so what? It's just one letter so
                # only one loop.
                new_location = dir_to_loc(loc)  # turns a direction into a location number
                if str(new_location) not in LOCATIONS:
                    if type(new_location) is dict and 'warning' in new_location:  # if it's a warning dict
                        ...
                        if boolean:
                            ...
                            if len(locations) > 1:
                                if went_to_already:  # if there is at least one already-went-to location in the list
                                    print(print(f"You went {' '.join(went_to_already)}. The rest of your direction commands will not be executed. "))
                                else:
                        ...
                        if len(locations) > 1 or len(locations[i]):
                            ...
                            return
                went_to_already.append(loc)  # this is after everything to make sure that it's only added to the places
            ...
            if going_to:
                ...
                if starting_location == new_location:
                    error_message = "You just went in circles. Going the directions you went took you back to the same " \
                                    "place." if len(locations) > 1 else "You're here already."
...
okays = ['Ok.', 'Alright', 'Okay.', 'Done.', 'Did it.', 'Done did it.', 'Complete.', 'It is done.', 'You did it.']
already_used_okays = []
